...
template <typename Tup // tuple
          ,
          typename T // field to find
          ,
          int Ind = 0, bool Same = ttl::meta::is_same<typename tup::element<Ind, Tup>::type::name, T>::value>
struct find
{
    enum
    {
...
template <typename Tup, typename T, int Ind> struct find<Tup, T, Ind, true>
{
    enum
    {
        value = Ind
...
//
template <typename T, typename Name> struct get_type
{
...
//
template <typename Name, // name of the function argument
...
template <typename P // named_pair<...> type
          >
...
//
template <typename Pair, typename T = typename Pair::type> struct argument
{
    typedef T type;
...
template <typename Pair, typename T> struct argument<Pair, const T &>
{
    typedef T type;
...
template <typename Pair, typename T> struct argument<Pair, T &>
{
    typedef T type;
...
template <typename R, TTL_FUNC_PAIR_DEF_PARAMS(TTL_MAX_NAMED_PARAMS)> struct named_params_function
{
};
...
#define TTL_FUNC_TUPLE_ARG(n, p)                                                                                       \
    tup::get<detail::find<data_tuple, typename P##n::name>::value>(d) = detail::named_pair_pnt<P##n>(&a##n);
#define TTL_FUNC_TUPLE_ARG_END(n, p)                                                                                   \
    tup::get<detail::find<data_tuple, typename P##n::name>::value>(d) = detail::named_pair_pnt<P##n>(&a##n);
#define TTL_FUNC_TUPLE_ARGS(n) TTL_REPEAT(n, TTL_FUNC_TUPLE_ARG, TTL_FUNC_TUPLE_ARG_END, p)

#define TTL_FUNC_OPERATOR(n, TotalArg)                                                                                 \
    template <TTL_TPARAMSX(n, P)> result_type operator()(TTL_FUNC_OPERATOR_ARGS(n))                                    \
    {                                                                                                                  \
        data_tuple d;                                                                                                  \
...
#define TTL_FUNC_TEMPLATE_DECLARATION(n)                                                                               \
    template <typename R, TTL_FUNC_PAIR_PARAMS(n)> struct named_params_function<R(TTL_FUNC_PAIR_ARGS(n))>              \
    {                                                                                                                  \
        typedef named_params_function this_t;                                                                          \
        ...
        TTL_FUNC_ARG_TYPES(n)                                                                                          \
                                                                                                                       \
        typedef tup::tuple<TTL_FUNC_TUPLE_TYPES(n)> data_tuple;                                                        \
                                                                                                                       \
        functor f_;                                                                                                    \
                                                                                                                       \
        named_params_function()                                                                                        \
        ...
        {                                                                                                              \
            ...
            return f_(TTL_FUNC_CALL_ARGS(n));                                                                          \
                                                                                                                       ...
                                                                                                                       \
        template <typename Name>                                                                                       \
        static detail::named_pair<Name, typename detail::get_type<data_tuple, Name>::type> arg(                        \
            typename data_holder<typename detail::get_type<data_tuple, Name>::type>::param_type d)                     \
        {                                                                                                              \
            return detail::named_pair<Name, typename detail::get_type<data_tuple, Name>::type>(d);                     \
        }
...
template <typename T, T Def> struct numeric_argument
{
    T d_;
