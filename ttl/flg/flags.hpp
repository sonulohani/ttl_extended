...
template <int Bits> struct bestfit<Bits, 1>
{
    typedef unsigned char type;
};

template <int Bits> struct bestfit<Bits, 2>
{
    typedef unsigned short type;
};

template <int Bits> struct bestfit<Bits, 3>
{
    typedef unsigned int type;
};

template <int Bits> struct bestfit<Bits, 4>
{
    typedef unsigned int type;
};
}; // namespace impl

template <typename T, int Bits = sizeof(int) * 8, typename Holder = typename impl::bestfit<Bits>::type> struct flags
{
    ...
    flags(T f1, T f2, T f3, T f4, T f5, T f6, T f7, T f8, T f9, T f10)
        : f_(f1 | f2 | f3 | f4 | f5 | f6 | f7 | f8 | f9 | f10)
    {
    }

    this_t &operator|=(const this_t &f)
    {
        f_ |= f.f_;
        return *this;
    }
    ...
    bool operator==(const this_t &l) const
    {
        return f_ == l.f_;
    }
    bool operator!=(const this_t &l) const
    ...
    bool test(const this_t l) const
    {
        return (f_ & l.f_) ? true : false;
    }
    bool test() const
    {
        return f_ != 0;
...
template <typename T, int Bits, typename Holder> flags<T, Bits, Holder> operator~(const flags<T, Bits, Holder> &l)
{
...
template <typename Pair, typename MapDirection = map_first2second> struct flag_mapper;

template <typename Pair> struct flag_mapper<Pair, map_first2second>
{
...
template <typename Pair> struct flag_mapper<Pair, map_second2first>
{
...
typename It::value_type::second_type map(const typename It::value_type::first_type f, It first, It end)
{
