...
namespace impl
{
template <typename F, typename V> struct variant_binary_visitor
{
    template <typename T1> struct visitor_functor
    {
        F &f_;
        T1 p1_;
        ...
        template <typename T2> void operator()(T2 p2)
        {
            f_(p1_, p2);
        }
    };
...
template <typename F, typename V> struct visitor_adapter_ref
{
    F &f_;
    V v_;
...
template <typename V> struct copier
{
    V &l_;
    const V &r_;
...
template <typename T, typename V> struct non_const_data
{
    enum
    {
...
///////////////////////////////////////////////////////////////////////
template <TTL_TPARAMS_DEF(TTL_MAX_TYPELIST_PARAMS, empty_type)> struct variant
{
    ...
    template <typename T> variant(const T &r) : which_(0), pnt_(0)
    {
    ...
    template <typename T>
    variant(T &r, typename selector<impl::non_const_data<T, this_t>::value, int,
    ...
    template <typename T> const ttl::data_holder<T> *get_holder() const
    {
        return static_cast<const ttl::data_holder<T> *>(pnt_);
    }

    template <typename T> ttl::data_holder<T> *get_holder()
    {
        return static_cast<ttl::data_holder<T> *>(pnt_);
    }

  private:
    template <int N> struct storage
    {
        union {
...
typename selector<meta::is_pointer<V *>::const_pnt, typename ttl::data_holder<T>::const_pointer,
                  typename ttl::data_holder<T>::pointer>::type
get(V *v)
{
    typedef meta::find_equivalent_type<T, typename V::list> found;
    if (v->which() != found::index)
        return 0;
    return &(v->template get_holder<typename found::type>()->get());
}

template <typename T, typename V>
typename selector<meta::is_reference<V &>::const_ref, typename ttl::data_holder<T>::const_return_type,
                  typename ttl::data_holder<T>::return_type>::type
get(V &v)
{
    typedef meta::find_equivalent_type<T, typename V::list> found;
    if (v.which() != found::index)
...
template <typename F, typename V> void apply_visitor(F &f, V &v)
{
    if (v.is_singular())
        return;
...
template <typename F, typename V1, typename V2> void apply_visitor(F &f, V1 &v1, V2 &v2)
{
