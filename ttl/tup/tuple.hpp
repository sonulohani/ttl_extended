...
#define TTL_TUPLE_CTOR(n, p)                                                                                           \
    tuple_data(TTL_TUPLE_CALL_PARAMS(n, p)) : head_(p1), tail_(TTL_ARGS_SX(TTL_DEC(n), p))                             \
    {                                                                                                                  \
    }

template <typename Types, int N, bool Stop = (N > meta::length<Types>::value)> struct tuple_param
{
    typedef typename ttl::data_holder<typename meta::get<Types, N - 1>::type>::param_type type;
};

template <typename Types, int N> struct tuple_param<Types, N, true>
{
    typedef empty_type type;
};

template <typename Types, int N = 0, bool Stop = (N >= meta::length<Types>::value)> struct tuple_data
{
...
template <typename Types, int N> struct tuple_data<Types, N, true>
{
    typedef tuple_data this_t;
    tuple_data()
    {
...
template <typename R, int N> struct get_field
{
    template <typename T> R operator()(T &t)
    {
        get_field<R, N - 1> g;
        return g(t.tail_);
    }
...
template <typename R> struct get_field<R, 0>
{
    template <typename T> R operator()(T &t)
    {
        return t.head_;
...
#define TTL_TUPLE_CTOR(n, p)                                                                                           \
    tuple(TTL_TUPLE_CALL_PARAMS(n, p)) : base(TTL_ARGSX(n, p))                                                         \
    {                                                                                                                  \
    }
...
template <int N, typename T> struct element
{
    typedef typename meta::get<typename T::types, N>::type type;
};

template <int N, typename T>
typename selector<meta::is_reference<T &>::const_ref, typename T::template const_return_type<N>::type,
                  typename T::template return_type<N>::type>::type
get(T &t)
{
    typedef typename selector<meta::is_reference<T &>::const_ref, typename T::template const_return_type<N>::type,
                              typename T::template return_type<N>::type>::type return_type;
