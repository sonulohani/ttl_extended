...
{
////////////////////////////////////////////////////////////
template <TTL_TPARAMS_DEF(TTL_MAX_TYPELIST_PARAMS, empty_type)> struct typelist_traits
{
    ...
    typedef typename selector<sizeof(ttl::data_holder<T1>) >= sizeof(ttl::data_holder<typename tail::largest_type>), T1,
                              typename tail::largest_type>::type largest_type;
};

template <> struct typelist_traits<TTL_LIST_ITEMS(TTL_MAX_TYPELIST_PARAMS, empty_type)>
{
...
#define TTL_META_TYPELIST_GET(n, t)                                                                                    \
    template <typename T> struct get<T, TTL_CNTDEC_##n>                                                                \
    {                                                                                                                  \
        enum                                                                                                           \
        {                                                                                                              \
            index = n - 1                                                                                              \
        };                                                                                                             \
        typedef typename T::t##n type;                                                                                 \
    };
...
template <TTL_TPARAMS_DEF(TTL_MAX_TYPELIST_PARAMS, empty_type)> struct typelist
{
...
///////////////////////////////////////////////////////////
template <typename L, int N, bool Ok = (N < length<L>::value)> struct get
{
...
template <typename L, int N> struct get<L, N, false>
{
...
{
    template <typename F> void operator()(size_t i, F &f)
    {
        if (i == N)
        {
...
template <typename L, int N> struct type_switch<L, N, true>
{
    template <typename F> void operator()(size_t i, F &f)
    {
...
//////////////////////////////////////////////////////////////
template <typename T, typename L, int N = 0, bool Stop = (N >= length<L>::value)> struct find_equivalent_type
{
  private:
    typedef impl::get<L, N> get_type;

    typedef typename selector<equivalent_types<typename get_type::type, T>::value, get_type,
                              find_equivalent_type<T, L, N + 1>>::type found;
...
template <typename T, typename L, int N> struct find_equivalent_type<T, L, N, true>
{
};

//////////////////////////////////////////////
template <typename T, typename L, int N = 0, bool Stop = (N >= length<L>::value)> struct find
{
  private:
    typedef impl::get<L, N> get_type;

    typedef typename selector<is_same<typename get_type::type, T>::value, get_type, find<T, L, N + 1>>::type found;
...
template <typename T, typename L, int N> struct find<T, L, N, true>
{
};
