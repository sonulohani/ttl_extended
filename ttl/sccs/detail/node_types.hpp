...
template <typename T> struct in_port
{
...
template <typename V, typename T> struct in_port_complete : in_port<V>
{
    typedef T type;
...
template <typename T> struct out_port
{
    typedef T message; // message variant
...
template <typename V, typename T> struct out_port_complete : out_port<V>
{
    typedef T type;
...
template <typename T> struct action_item
{
    typedef T action;
    action a_;
    std::string n_;

    action_item(action a, const std::string &name) : a_(a), n_(name)
    {
    }
