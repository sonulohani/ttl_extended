...
template <typename M> struct agent
{
    ...
    */
    ...
    typedef var::variant<in_port_pnt, out_port_pnt, action_item_pnt, repeat_pnt

                         ,
                         detail::sum_operator, detail::product_operator, detail::prefix_operator, detail::root,
                         detail::stop>
        content;
    ...
    template <typename P> this_t *in(const P &d = P())
    {
    ...
    template <typename P> this_t *out(const P &d = P())
    {
    ...
    {
        detail::add_action_item_visitor<this_t, action_item_pnt> v(
            *this, action_item_pnt(new detail::action_item<action>(d, name)));
        var::apply_visitor(v, c_);
        return this;
    }
    /*
    ...
    */
...
#endif
    {
        ...
        template <typename T> void operator()(const T &d)
        {
        }
    };
  ...
  private:
    ...
    {
        detail::add_action_item_visitor<this_t, action_item_pnt> v(*this, a);
        var::apply_visitor(v, c_);
        return this;
    }
...
template <typename A> A *start()
{
    return new A();
}

template <typename M>
boost::shared_ptr<agent<M>> operator+(boost::shared_ptr<agent<M>> l, boost::shared_ptr<agent<M>> r)
{
...
template <typename A, typename TT = boost_threads> struct runtime
{
